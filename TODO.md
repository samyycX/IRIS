...
FEATURE:
0. [x] 让aliases中所有中文都有对应的中文拼音作为别名，通过LLM处理时实现。
1. [x] 一个综合管理本地数据的服务，提供接口给本项目各类未来服务保存数据的功能(json)，支持本地部署和docker挂载的/data文件夹（通过env判断docker环境，如果是docker环境使用/docker，本地就存当前目录的./data文件夹）
2. [x] 将配置移到前端，动态更新，存在本地（用上面那个服务），有版本控制，migration。并且让prompt也可以自定义（不需要wuwa预设了）。前端需要做分组分类，人性化的配置页面。(Gemini 3 pro)将Neo4J和LLM数据源分离（可以独立创建多个Neo4J和LLM配置，在配置文件里可以选择所有定义好的数据源作为embedding模型，llm模型等）
3. [x] 前后端需要做验证，环境变量可以设置入口密码，使用argon2id做密码增强，前端必须输入密码才能进入管理页面，后端也要做校验
4. [x] 删除InMemoryJobStore，保留vectorindexjob
5. [x] 一套APIKey生成，统计，管理逻辑（后端GPT5,前端Gemini3pro），用于调用未来的查询接口。用本地数据服务进行存储和读取。
6. [x] 提供一套查询接口，可以选择数据源，实现查询neo4j的entity,Source对象，再加上综合性的查找
